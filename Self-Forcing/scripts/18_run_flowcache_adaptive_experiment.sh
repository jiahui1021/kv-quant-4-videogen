...
RUN_NAME="${RUN_NAME:-flowcache_adaptive_smoke}"
...
FLOWCACHE_IMPORTANT_OLD_RATIO="${FLOWCACHE_IMPORTANT_OLD_RATIO:-0.20}"
...
cat > "${RUN_ROOT}/run_meta.json" <<EOF
{
  ...
  "run_id": "${RUN_ID}",
  "run_root": "${RUN_ROOT}",
  "experiment_type": "flowcache_adaptive_experiment",
  ...
  "flowcache_profile_min_scale": ${FLOWCACHE_PROFILE_MIN_SCALE},
  "flowcache_profile_max_scale": ${FLOWCACHE_PROFILE_MAX_SCALE}
}
EOF
...
for method in "${METHODS[@]}"; do
  ...
  elif [[ "${method}" == "FLOWCACHE_HYBRID_INT2" || "${method}" == "FLOWCACHE_ADAPTIVE_INT2" ]]; then
    base_cmd+=(
      --flowcache-recent-ratio "${FLOWCACHE_RECENT_RATIO}"
...
echo "FlowCache adaptive experiment completed for ${RUN_ID}."
