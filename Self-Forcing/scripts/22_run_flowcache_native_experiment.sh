...
RUN_NAME="${RUN_NAME:-flowcache_native_smoke}"
...
FLOWCACHE_PRUNE_RETAINED_OLD_RATIO="${FLOWCACHE_PRUNE_RETAINED_OLD_RATIO:-0.45}"
FLOWCACHE_PRUNE_REFRESH_GAP_CHUNKS="${FLOWCACHE_PRUNE_REFRESH_GAP_CHUNKS:-1}"
FLOWCACHE_NATIVE_REL_L1_THRESH="${FLOWCACHE_NATIVE_REL_L1_THRESH:-1.50}"
FLOWCACHE_NATIVE_WARMUP_STEPS="${FLOWCACHE_NATIVE_WARMUP_STEPS:-0}"
...
cat > "${RUN_ROOT}/run_meta.json" <<EOF
{
  ...
  "run_id": "${RUN_ID}",
  "run_root": "${RUN_ROOT}",
  "experiment_type": "flowcache_native_experiment",
  "flowcache_native_rel_l1_thresh": ${FLOWCACHE_NATIVE_REL_L1_THRESH},
  "flowcache_native_warmup_steps": ${FLOWCACHE_NATIVE_WARMUP_STEPS},
...
for method in "${METHODS[@]}"; do
  ...
  if [[ "${method}" == FLOWCACHE_SOFT_PRUNE_* || "${method}" == "FLOWCACHE_NATIVE_SOFT_PRUNE_INT4" ]]; then
    base_cmd+=(
      --flowcache-recent-ratio "${FLOWCACHE_RECENT_RATIO}"
  ...
  if [[ "${method}" == FLOWCACHE_NATIVE* ]]; then
    base_cmd+=(
      --flowcache-native-rel-l1-thresh "${FLOWCACHE_NATIVE_REL_L1_THRESH}"
      --flowcache-native-warmup-steps "${FLOWCACHE_NATIVE_WARMUP_STEPS}"
    )
  fi
...
for method in "${METHODS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" PYTHON_BIN="${EVAL_PYTHON}" RUN_ROOT="${RUN_ROOT}" \
    "${ROOT_DIR}/scripts/03_eval_vbench.sh" "${method}" "${RUN_ROOT}/videos/${method}" "${ROOT_DIR}/prompts/moviegen_128.txt" "${RUN_ROOT}/metrics/vbench_${method}" "${RUN_ROOT}/metrics/vbench_${method}.json" \
