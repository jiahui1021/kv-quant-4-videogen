...
RUN_NAME="${RUN_NAME:-flowcache_soft_prune_smoke}"
...
cat > "${RUN_ROOT}/run_meta.json" <<EOF
{
  ...
  "run_id": "${RUN_ID}",
  "run_root": "${RUN_ROOT}",
  "experiment_type": "flowcache_soft_prune_experiment",
...
if [[ -n "${METHODS_CSV}" ]]; then
  ...
  for idx in "${!METHODS[@]}"; do
    METHODS[$idx]="$(echo "${METHODS[$idx]}" | xargs)"
  done
else
  METHODS=(
    BF16
...
for method in "${METHODS[@]}"; do
  ...
  if [[ "${method}" == "AGE_TIER_INT2" ]]; then
  ...
  elif [[ "${method}" == "FLOWCACHE_ADAPTIVE_INT2" || "${method}" == FLOWCACHE_PRUNE_* || "${method}" == FLOWCACHE_SOFT_PRUNE_* ]]; then
    base_cmd+=(
      --flowcache-recent-ratio "${FLOWCACHE_RECENT_RATIO}"
    ...
    if [[ "${method}" == FLOWCACHE_PRUNE_* || "${method}" == FLOWCACHE_SOFT_PRUNE_* ]]; then
      base_cmd+=(
...
if [[ "${HAS_BF16}" == "1" && -d "${RUN_ROOT}/videos/BF16" ]]; then
  for method in "${METHODS[@]}"; do
    if [[ "${method}" == "BF16" ]]; then
      continue
    fi
  ...
  echo "[fidelity] skipped (BF16 reference not present in METHODS)"
fi

"${INFER_PYTHON}" "${ROOT_DIR}/scripts/05_summarize_results.py" --results-root "${RUN_ROOT}" --methods "${METHODS[@]}"

echo "FlowCache soft-prune experiment completed for ${RUN_ID}."
