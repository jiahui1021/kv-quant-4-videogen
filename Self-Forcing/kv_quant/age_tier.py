...
class AgeTierQuantizer(KVQuantizer):
    """
    Recency-aware tiered precision:
    - recent tokens use higher precision
    - old tokens use lower precision
    """
    ...
    def __init__(
        ...
        super().__init__(bits=bits, block_size=block_size, name=f"AGE_TIER_INT{bits}")
        self._recent_quantizer = self._METHOD_TO_CLASS[self.recent_method](bits=self.recent_bits, block_size=block_size)
        self._old_quantizer = self._METHOD_TO_CLASS[self.old_method](bits=self.bits, block_size=block_size)
    ...
    def diagnostics(self) -> Dict[str, float]:
        ...
        if events == 0:
            return {}
        return {
            "age_tier_events": float(events),
            "age_tier_avg_recent_ratio": float(sum(self._recent_ratio_history) / events),
            "age_tier_config_recent_ratio": float(self.recent_ratio),
            "age_tier_recent_bits": float(self.recent_bits),
            "age_tier_old_bits": float(self.bits),
        }
