...
parser.add_argument("--num_output_frames", type=int)
parser.add_argument("--local_attn_size", type=int)
parser.add_argument("--num_samples", type=int)
parser.add_argument("--seed", type=int)
parser.add_argument("--prompt_index", type=int)
parser.add_argument(
    "--profile",
    action=argparse.BooleanOptionalAction,
    default=None,
    help="Override the config profiling flag.",
)
args = parser.parse_args()

config = OmegaConf.load(args.config_path)

# Keep the original YAML files reusable while allowing the 717-frame SF/CF
# smoke runner to freeze all workload-specific values from the command line.
for name in ("checkpoint_path", "model_name", "data_path", "output_folder"):
    value = getattr(args, name)
    if value is not None:
        target = "generator_ckpt" if name == "checkpoint_path" else name
        setattr(config, target, value)
for name in ("num_output_frames", "num_samples", "seed"):
    value = getattr(args, name)
    if value is not None:
...
if args.profile is not None:
    config.profile = bool(args.profile)
# WanDiffusionWrapper receives model_kwargs, while the historical configs keep
# model_name at the top level. Pass the resolved absolute path through both
# representations so this runner works outside the original machine path.
if getattr(config, "model_name", None):
    config.model_kwargs.model_name = str(config.model_name)
...
if args.prompt_index is not None:
    if args.prompt_index < 0 or args.prompt_index >= len(dataset):
        raise ValueError(
            f"prompt_index={args.prompt_index} is outside dataset of size {len(dataset)}"
        )
    dataset = Subset(dataset, [args.prompt_index])
        ...
        compression.update(
            {
                ...
                if "self_forcing" in str(args.config_path)
                else "causal_forcing",
                "prompt_idx": int(idx),
                "pixel_frames": int(config.num_output_frames) * 4 - 3,
                "latent_frames": int(config.num_output_frames),
                "method": "Forcing-KV",
            }
        )
        report_path = Path(config.output_folder) / "efficiency" / f"compression_ratio_{idx}.json"
        write_report(report_path, compression)
        print(
            f"[ForcingKV] compression ratio: "
            f"{compression['compression_ratio']!s} "
            f"({compression['bf16_equivalent_bytes']} / "
            f"{compression['resident_kv_bytes']} bytes)"
        )
