...
def _pretrained_root() -> Path:
    return Path(
        os.environ.get(
            "FORCING_KV_PRETRAINED_ROOT",
            "/ycji/code/Forcing-KV/pretrained",
        )
    ).expanduser()
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        model_root = _pretrained_root() / "Wan2.1-T2V-1.3B"
        self.text_encoder.load_state_dict(
            torch.load(str(model_root / "models_t5_umt5-xxl-enc-bf16.pth"),
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=str(
                _pretrained_root() / "Wan2.1-T2V-1.3B/Wan2.1_VAE.pth"
            ),
            z_dim=16,
        ).eval().requires_grad_(False)
